...
def _simulate_match(
    ...
    Uses VaR/CVaR bounded noise (3% VaR at 97th pct) — Trial 1 preserved the
    original call signature but now applies bounded perturbation instead of
    unconstrained Gaussian (σ=0.08) from prior versions.
    Draws are always resolved to a winner (50/50 extra time) so the return
    list never contains 'draw', matching the original contract.
    """
    winners = []
    for _ in range(n):
        # Always call as knockout=True so simulate_match_var never returns 'draw'
        # (original trial-1 logic resolved draws internally via 50/50 extra time)
...
def _simulate_group(
    ...
    Uses VaR/CVaR bounded simulation — wraps simulate_group_var."""
